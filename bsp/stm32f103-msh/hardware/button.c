			...
			{	
				if((buttons[BUTTON_3].status & BUTTON_PRESSED) && (buttons[BUTTON_4].status & BUTTON_PRESSED))
				{
					...
					{//save data
						...
						else if(curr_status.value.mode == MODE_QJ)//ģʽ2
						{
						...
						curr_status.value.sys_set &= 0x7F;//�˳�����
					}
					else
					{	
						if(curr_status.value.mode == MODE_LUM)//ģʽ0
						{
							if(curr_status.value.lum_grade==(LUM_GRADE_NUMB-1))//max level
							{
								Recover_To_MAX(MODE_LUM);//�ָ��նȣ�ȥ������Ӱ��
								curr_status.value.sys_set |= 0x80;//ʹ������
							}
							if(curr_status.value.lum_grade==0)//min level	
							{
								Recover_To_MIN(MODE_LUM);//�ָ��նȣ�ȥ������Ӱ��
								curr_status.value.sys_set |= 0x80;//ʹ������
							}
						}
						else if(curr_status.value.mode == MODE_QJ)//ģʽ0
						{
							if(curr_status.value.qj_grade ==(QJ_GP_NUMB-1))//max level
							{
								Recover_To_MAX(MODE_QJ);//�ָ��նȣ�ȥ������Ӱ��
								curr_status.value.sys_set |= 0x80;//ʹ������
							}
							if(curr_status.value.qj_grade==0)//min level	
							{
								Recover_To_MIN(MODE_QJ);//�ָ��նȣ�ȥ������Ӱ��
								curr_status.value.sys_set |= 0x80;//ʹ������
							}
						}
					}
