...
//*str1:字符串1指针
//*str2:字符串2指针			   
void db_strcopy(u8*str1,u8 *str2)
{
	while(1)
	{										   
		*str2=*str1;	//拷贝
...
//返回值:0,成功;其他,失败.	  
u8 db_get_cmdname(u8*str,u8*cmdname,u8 *nlen,u8 maxlen)
{
	*nlen=0;
...
//返回值:0,成功;其他,错误代码.
u8 db_get_fname(u8*str,u8*fname,u8 *pnum,u8 *rval)
{
	u8 res;
	u8 fover=0;	  //括号深度
	u8 *strtemp;
	...
	if(pcnt)//接收完了
	{
		...
		if(db_strcmp(fpname,"void")==0)*rval=0;//不需要返回值
		else *rval=1;							   //需要返回值
		pcnt=0;
	} 
	res=0;
	...
	{  
		...
		if(*strtemp==' '||*strtemp=='*')
		{
			nchar=db_search_nextc(strtemp);		//获取下一个字符
	...
	if(parmnum==1)//只有1个参数.
	{
		...
		if(db_strcmp(fpname,"void")==0)parmnum=0;//参数为void,表示没有参数.
	}
	*pnum=parmnum;	//记录参数个数
	*fname='\0';	//加入结束符
	return res;		//返回执行结果
...
u8 db_get_aparm(u8 *str,u8 *fparm,u8 *ptype)
{
	u8 i=0;
	u8 enout=0;
	u8 type=0;//默认是数字
...
//num:第num个参数,范围0~9.
//返回值:该参数的起始地址
u8 db_get_parmpos(u8 num)
{
	u8 temp=0;
	u8 i;
	for(i=0;i<num;i++)temp+=db_dev.plentbl[i];
	return temp;
}
//从str中得到函数参数
//str:源字符串;
...
//返回值:0,成功;其他,错误代码.
u8 db_get_fparam(u8*str,u8 *parn)
{	
	u8 i,type;  
	u32 res;
	u8 n=0;
	u8 len;
	u8 tstr[MAX_DB_LEN+1];//字节长度的缓存,最多可以存放PARM_LEN个字符的字符串
	for(i=0;i<MAX_DB_PARM_NO;i++)db_dev.plentbl[i]=0;//清空参数长度表
	while(*str!='(')//偏移到参数开始的地方
	{
		str++;											    
		if(*str=='\0')return DB_FUNCERR;//遇到结束符了
	}
	str++;//偏移到"("之后的第一个字节
	while(1)
	{
		i=db_get_aparm(str,tstr,&type);	//得到第一个参数  
		str+=i;								//偏移
		switch(type)
		{
			case 0:	//数字
				...
				{					    
					i=db_str2num(tstr,&res);	//记录该参数	 
					if(i)return DB_PARMERR;		//参数错误.
					*(u32*)(db_dev.parm+db_get_parmpos(n))=res;//记录转换成功的结果.
					db_dev.parmtype&=~(1<<n);	//标记数字
					db_dev.plentbl[n]=4;		//该参数的长度为4  
					n++;							//参数增加  
					if(n>MAX_DB_PARM_NO)return DB_PARMOVER;//参数太多
				}
				break;
			case 1://字符串	 	
				len=db_strlen(tstr)+1;	//包含了结束符'\0'
				db_strcopy(tstr,&db_dev.parm[db_get_parmpos(n)]);//拷贝tstr数据到db_dev.parm[n]
				db_dev.parmtype|=1<<n;	//标记字符串 
				db_dev.plentbl[n]=len;	//该参数的长度为len  
				n++;
				if(n>MAX_DB_PARM_NO)return DB_PARMOVER;//参数太多
				break;
			case 0XFF://错误
				return DB_PARMERR;//参数错误	  
		}
		if(*str==')'||*str=='\0')break;//查到结束标志了.
	}
	*parn=n;	//记录参数的个数
	return DB_OK;//正确得到了参数
}
